package za.co.ltsmwebsystem.controller;
...
import za.co.ltsmwebsystem.bean.EmployeeBean;
import za.co.ltsmwebsystem.dao.impl.PasswordEncrypt;
...
import za.co.ltsmwebsystem.service.LoginAttemptServiceInt;
import za.co.ltsmwebsystem.service.UserLogDetailsServiceInt;
		...
		if(employee != null&& employee.getStatus().equalsIgnoreCase("ACTIVE")){
			...
			if(employee.isFirstTimeLogin()==true && employee.getEmail().equals(userName)&& employee.getPassword().equals(password)){
				...
				if(numberOfDays > 65 && numberOfDays < 75){
					...
					else if(employee.getRole().equalsIgnoreCase("Book Seller") && employee.getEmail().equals(userName)&& employee.getPassword().equals(password))
					{
						String userSessionID =session.getId();
						session.setAttribute("sessionID", userSessionID);
						if(details !=null){
	...
	public ModelAndView loadHomePages() {
		...
		if(userName !=null){
			if(userName.getRole().equalsIgnoreCase("Manager") || (userName.getRole().equalsIgnoreCase("Admin"))){
				    model.setViewName("home");
			}else if(userName.getRole().equalsIgnoreCase("Book Seller")){
					model.setViewName("technicianHome");				
